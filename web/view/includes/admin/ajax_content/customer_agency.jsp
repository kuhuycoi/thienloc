...
<c:set var="PAGINATION" value="${sessionScope['AGENCY_PAGINATION']}"/>
    ...
    <table class="table table-condensed table-hover table-valign-midle table-grid-view table-align-center">
        <thead>
            <tr controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.ORDER_DATA}/'/>">
                ...
                <th class="col-md-2" column="customerId.userName">Username <span class="${PAGINATION.orderColmn=='customerId'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-2" column="rankAgencyID.name">Gói đầu vào <span class="${PAGINATION.orderColmn=='rankAgencyID'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-3" column="dateCreated">Thời gian bắt đầu <span class="${PAGINATION.orderColmn=='dateCreated'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-3" column="dateEnd">Thời gian kết thúc <span class="${PAGINATION.orderColmn=='dateEnd'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
            </tr>
        </thead>
        <tbody>
            <c:forEach items="${PAGINATION.displayList}" var="admin">
                <tr>
                    ...
                    <td>${admin.customerId.userName}</td>
                    <td>${admin.rankAgencyID.name}</td>
                    <td>${f:formatTime(admin.dateCreated)}</td>
                    <td>${f:formatTime(admin.dateEnd)}</td>
                </tr>
            </c:forEach>
        </tbody>
    </table><!-- end table -->
