    ...
    <table class="table table-condensed table-hover table-valign-midle table-grid-view table-align-center table-bordered">
        <thead>
            <tr controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.ORDER_DATA}/'/>">
                ...
                <th class="col-md-4" column="name">Gói thưởng <span class="${PAGINATION.orderColmn=='name'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-1" column="price">Giá tiền <span class="${PAGINATION.orderColmn=='price'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-1" column="pricePv">Giá Pv <span class="${PAGINATION.orderColmn=='pricePv'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                ...
                <th class="col-md-1" column="time">Thời gian <span class="${PAGINATION.orderColmn=='time'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="th-action external"></th>
            </tr>
        </thead>
        <tbody>
            <c:forEach items="${PAGINATION.displayList}" var="c">
                <tr>
                    <td><input type="checkbox" /></td>
                    <td>${c.id}</td>
                    <td>${c.name}</td>
                    <td>${f:formatCurrency(c.price)}</td>
                    <td>${f:formatCurrency(c.pricePv)}</td>
                    <td>${f:formatPercent(c.percent)}</td>
                    <td>${c.time}</td>
                    <td>
