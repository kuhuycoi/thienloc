...
<c:set var="PAGINATION" value="${sessionScope['GALERY_CATEGORY_PAGINATION']}"/>
    ...
    <table class="table table-condensed table-hover table-valign-midle table-grid-view table-align-center">
        <thead>
            <tr data-toggle="tooltip" data-placement="top" data-title="Click vào cột để sắp xếp theo cột đó, click lần nữa để sắp xếp ngược lại." controller='<c:url value="${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.ORDER_DATA}/"/>'>
                ...
                <th class="col-md-4 text-left" column="name">Tên Album <span class="${PAGINATION.orderColmn=='name'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-1 external">Ảnh đại diện</th>
                ...
                <th class="col-md-1" column="orderNumber">Thứ tự <span class="${PAGINATION.orderColmn=='isShow'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-md-2" column="createdAdm.userName">Người tạo <span class="${PAGINATION.orderColmn=='createdAdm.userName'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
        ...
        <div class="btn-group btn-group-sm">
            ...
            <ul class="dropdown-menu dropdown-menu-right dropdown-menu-multiple" role="menu">
                <li>
                    <a controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.DELETE}'/>">Xóa</a>
                </li>
            </ul>
        </div>
    </th>
...
<tbody>
    <c:forEach items="${PAGINATION.displayList}" var="news">
        <tr>
            ...
            <td><img src="<c:url value="${news.titleImg}" />" /></td>
            <td>${news.isShow?"Hiện":"Ẩn"}</td>
            <td>${news.orderNumber}</td>
            <td>${news.createdAdm.userName}</td>
            <td>${f:formatTime(news.createdDate)}</td>
            <td>
                <div class="btn-group btn-group-sm">
                    <a type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"><i class="ti-settings"></i></a>
                    <ul class="dropdown-menu dropdown-menu-right dropdown-menu-action" role="menu">
                        <li><a class="btn-open-modal" controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.VIEW_EDIT}/${news.id}'/>">Sửa</a></li>
                        <li><a controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.HIDE}/${news.id}'/>">${news.isShow?"Ẩn":"Hiện"}</a></li>
                        <li><a controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.DELETE}/${news.id}'/>">Xóa</a></li>
                    </ul>
                </div>
            </td>
        </tr>
