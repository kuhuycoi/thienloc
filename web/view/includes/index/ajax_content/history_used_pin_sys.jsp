    ...
    <table class="table table-bordered table-hover table-valign-midle table-grid-view table-align-center">
        <thead>
            <tr controller="<c:url value='${PAGINATION.ROOT_CONTROLLER}${PAGINATION.childrenController}${PAGINATION.grandController}${PAGINATION.ORDER_DATA}/'/>">
                <th class="col-xs-3 text-left" column="pinNumber">Mã Pin <span class="${PAGINATION.orderColmn=='pinNumber'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-xs-3 text-left" column="pinType">Gói PV <span class="${PAGINATION.orderColmn=='pinType'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
                <th class="col-xs-3" column="usedDate">Thời gian nạp <span class="${PAGINATION.orderColmn=='usedDate'?(PAGINATION.asc?'fa fa-caret-up':'fa fa-caret-down'):''}"></span></th>
