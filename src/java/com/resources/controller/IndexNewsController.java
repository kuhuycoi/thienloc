...
public class IndexNewsController {
    ...
    public ModelAndView getNewsView(@PathVariable(value = "id") Integer id, ModelMap mm, HttpSession session) {
        NewsCategories category = (NewsCategories) new NewsCategoriesFacade().find(id);
        NewsPagination newsPagination = new NewsPagination(category.getName(), "/news", id);
    ...
    @ResponseBody
    public ModelAndView gotoNewsView(@PathVariable("page") int page, ModelMap mm, HttpSession session) {
        NewsPagination newsPagination = (NewsPagination) session.getAttribute("INDEX_NEWS");
        if (newsPagination == null) {
            newsPagination = new NewsPagination(null, "/news", 1);
        }
        newsPagination.setCurrentPage(page);
        return newsView(newsPagination, session);
    }
